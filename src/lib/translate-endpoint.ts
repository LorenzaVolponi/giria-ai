...
export async function handleTranslatePost(request: NextRequest) {
  try {
    ...
    if (!text) {
      return withSecurityHeaders(
        NextResponse.json({ error: "Envie um texto/gíria para tradução." }, { status: 400 }),
      );
    }
    ...
    return withSecurityHeaders(
      NextResponse.json({ error: "Não foi possível processar a tradução agora." }, { status: 500 }),
    );
  }
}
