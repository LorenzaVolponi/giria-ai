...
export async function POST(request: NextRequest) {
  try {
    ...
    if (resolvedMode === "list") {
      ...
      return withSecurityHeaders(applyCommonResponseHeaders(applyLegacyDeprecationHeaders(listRes), resolvedMode));
    }

    if (resolvedMode === "single") {
      const singleRes = NextResponse.json({ mode: resolvedMode, response });
      return withSecurityHeaders(applyCommonResponseHeaders(applyLegacyDeprecationHeaders(singleRes), resolvedMode));
    }
    ...
    return withSecurityHeaders(applyCommonResponseHeaders(applyLegacyDeprecationHeaders(defaultRes), resolvedMode));
  } catch (error: unknown) {
    const message =
