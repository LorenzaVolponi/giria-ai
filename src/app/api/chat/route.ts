...
export async function POST(request: NextRequest) {
  try {
    ...
    const resolvedMode: ChatResponseMode =
      responseMode ??
    ...
    const finalizeResponse = (res: NextResponse, mode: ChatResponseMode): NextResponse =>
      withSecurityHeaders(applyLegacyDeprecationHeaders(applyCommonResponseHeaders(res, mode)));
    ...
    if (resolvedMode === "single") {
      return finalizeResponse(NextResponse.json({ mode: resolvedMode, response }), resolvedMode);
    }
