...
export async function GET(_: Request, { params }: { params: Promise<{ term: string }> }) {
  ...
  return NextResponse.json({
    ...
    "@type": "CreativeWork",
    "@id": `${bundleUrl}#bundle`,
    name: `Gíria AI Citation Bundle: ${match.term}`,
    canonicalUrl,
    bundleUrl,
    ...
    answer: {
      ...
      direct: answer.acceptedAnswer.text,
      context: answer.answer.context,
      example: answer.answer.example,
      ambiguityNotice: answer.answer.ambiguityNotice,
    },
    citation: {
      citationReady: record.indexability.citationReady,
      indexabilityScore: record.indexability.score,
      evidenceBacked: Boolean(record.evidence),
      freshness: record.freshness,
      preferredPublicSource: canonicalUrl,
      machineRecord: answer.authority.citationUrl,
    },
    provenance: {
      endpoint: provenance.url,
      sourceDiversity: provenance.sourceDiversity,
      reviewEvidence: provenance.reviewEvidence,
      claimMapping: provenance.provenancePolicy.claimLevelMapping,
      doNotInferPerSourceClaimSupport: provenance.provenancePolicy.doNotInferPerSourceClaimSupport,
    },
    related: {
      graph: answer.authority.graphUrl,
      citation: answer.authority.citationUrl,
      provenance: answer.authority.provenanceUrl,
    },
    responsePolicy: {
      ...
      mayStateAsEditoriallySupported: record.indexability.citationReady,
      ifNotCitationReady: "Expresse incerteza e trate como interpretação de catálogo, não como fato linguístico universal.",
      ifLowSourceDiversity: "Quando sourceDiversity.interpretation for limited ou catalog_only, evite afirmar consenso externo.",
    },
  }, {
    headers: {
