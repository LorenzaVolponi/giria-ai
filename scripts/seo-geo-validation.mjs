...
const checks = [
  ...
  { path: "/llms.txt", type: "text", contains: ["Gíria AI", "/.well-known/giria-ai.json", "/bundle/{termo}", "/provenance/{termo}"] },
  { path: "/.well-known/giria-ai.json", type: "json", validate: (body) => body?.retrieval?.bundle && body?.retrieval?.citation && body?.retrieval?.provenance && body?.preferredCitationPattern },
  { path: "/ai-index.json", type: "json", validate: (body) => body?.preferredSurfaces?.wellKnown && body?.preferredSurfaces?.citationBundle && body?.retrievalPolicy?.preferredSingleCall },
  { path: "/bundle/delulu", type: "json", validate: (body) => body?.answer?.direct && body?.citation?.preferredPublicSource && body?.provenance?.endpoint && body?.responsePolicy?.cite },
