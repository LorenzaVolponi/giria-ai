...
const checks = [
  ...
  { path: "/llms.txt", type: "text", contains: ["Gíria AI", "/knowledge.json", "/api/graph", "/answers.json", "/answer/{termo}", "/authority.json"] },
  { path: "/ai-index.json", type: "json", validate: (body) => body?.preferredSurfaces?.directAnswer && body?.preferredSurfaces?.bulkAnswers && body?.preferredSurfaces?.topicalAuthority && body?.retrievalPolicy?.definitionQuestion && body?.retrievalPolicy?.topicalScope },
