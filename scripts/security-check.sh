...
while IFS= read -r -d '' file; do
  ...
  if grep -IEn -e "$PATTERN" -- "$file"; then FOUND_SECRET=1; fi
done < <(git ls-files -z)
if [[ "$FOUND_SECRET" -eq 1 ]]; then echo "[sec][CRITICO] Possível segredo encontrado."; exit 1; else echo "[sec] Nenhum padrão crítico encontrado."; fi
...
set +e; npm audit --json > "$AUDIT_FILE"; AUDIT_EXIT=$?; set -e
node - "$AUDIT_FILE" <<'NODE'
const fs = require("fs");
const report = JSON.parse(fs.readFileSync(process.argv[2], "utf8"));
const blockingSeverities = new Set(["high", "critical"]);
const allowlistedAdvisories = new Set(["GHSA-ggr8-5vv4-36mx"]);
const prismaChain = new Set(["deepmerge-ts", "@prisma/config", "prisma"]);
...
function advisoryIdsFor(name, seen = new Set()) {
  ...
  return vuln.via.flatMap((item) => {
    if (typeof item === "string") return advisoryIdsFor(item, seen);
...
for (const [name, vuln] of Object.entries(vulnerabilities)) {
  ...
  const advisoryIds = [...new Set(advisoryIdsFor(name))];
  const allowed = prismaChain.has(name) && advisoryIds.length > 0 && advisoryIds.every((id) => allowlistedAdvisories.has(id));
  if (allowed) {
    console.warn(`[sec][TEMP] Advisory ${advisoryIds.join(", ")} propagado por ${name} aceito temporariamente: cadeia prisma -> @prisma/config -> deepmerge-ts; acompanhar até atualização do lockfile.`);
    continue;
  }
  blocking.push(`${name} (${vuln.severity})`);
}
if (blocking.length) { console.error(`[sec][CRITICO] Vulnerabilidades high/critical não permitidas: ${blocking.join(", ")}`); process.exit(1); }
NODE
if [[ "$AUDIT_EXIT" -ne 0 ]]; then echo "[sec] npm audit reportou findings; somente exceções explícitas e verificadas foram aceitas."; fi
...
if [[ -z "${ALLOWED_ORIGIN:-}" ]]; then echo "[sec][WARN] ALLOWED_ORIGIN não definido (defina em produção)."; fi
